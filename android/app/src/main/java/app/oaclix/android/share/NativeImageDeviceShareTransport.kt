...
internal data class NativeImageDeviceShareResult(
    val targetDeviceId: String,
    val transferId: String,
    val itemId: String,
)

internal class NativeImageDeviceShareTransport(
    baseUrl: String,
    ...
    fun send(
    ...
    ): NativeImageDeviceShareResult {
        require(DEVICE_ID_PATTERN.matches(targetDeviceId)) { "Destino inválido" }
        require(bytes.isNotEmpty()) { "La imagen está vacía" }
        require(bytes.size.toLong() <= ImageTransferPolicy.CLOUD_MAX_IMAGE_BYTES) { "La copia cloud supera 10 MB" }
        require(mimeType in SUPPORTED_MIME_TYPES) { "Formato de imagen no compatible" }
        require(timeoutMs in 5_000L..120_000L) { "Timeout inválido" }
        ...
        val result = AtomicReference<NativeImageDeviceShareResult?>(null)
                ...
                when (message.optString("type")) {
                    ...
                    "presence" -> {
                        ...
                        val relay = JSONObject()
                            .put("type", "device-image-transfer")
                            .put("targetDeviceId", targetDeviceId)
                            .put("transfer", transfer)
                        if (!webSocket.send(relay.toString())) {
                            failure.compareAndSet(null, IOException("No se pudo enviar la imagen"))
                            completed.countDown()
                        } else {
                            sent.set(true)
                        }
                    }
                    "device-image-transfer-ack" -> {
                        ...
                        when (ack.optString("status")) {
                            "stored" -> result.compareAndSet(
                                null,
                                NativeImageDeviceShareResult(targetDeviceId, transferId, itemId),
                            )
                            "expired" -> failure.compareAndSet(null, IOException("La imagen venció antes de guardarse"))
                            "rejected" -> failure.compareAndSet(null, IOException("El otro dispositivo rechazó la imagen"))
                            else -> return
                        }
                        completed.countDown()
                    }
        ...
        try {
            ...
            return result.get() ?: throw IOException("El dispositivo no confirmó la imagen")
        } finally {
    ...
    internal companion object {
        ...
        private val SUPPORTED_MIME_TYPES = setOf("image/png", "image/jpeg", "image/webp", "image/gif")
